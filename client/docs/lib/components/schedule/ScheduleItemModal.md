...
| `onTimer` | `((gameId: string) => void) \| undefined` | Called when the "timer" footer action is pressed (only enabled when editing an existing item; passes `item.gameId`, which may itself be empty for a not-yet-created timer/game). |
