...
| `runWipeGroup` | `(group: WipeGroup, local: WipeStatusMap): Promise<void>` | Marks `group` as the `activeWipeGroup`, then calls [`deleteItems`](../../import/wipe-service.md) for it, updating `local` and `wipeStatuses` per item as each delete resolves. The third argument to `deleteItems` is a "should continue" predicate checking `mountedRef`/`cancelRequestedRef` between items. |
