...
| `game` | Game overview screen: the state badge (`state.live`/`planned`/`finished`), the seating card (`seating.title`, `seating.table` for the big table-number tile, `seating.seat`), the table-bell hint lines (`bell.idle`/`ringing`/`acknowledged`/`notActive`/`finished`/`noTable`, the first two interpolated with `{{elapsed}}`), action buttons (`lottery`, `rules`, `tableBell`, `timer`, `results`) with nested `confirmRing`/`confirmDismiss` dialogs, plus the player-color-setup sub-screen (`colorSetup.*`: corner labels, duplicate-player validation, fallback label) | Game overview screen, player color setup |
| `participants` | Participant directory: screen `title`, search placeholder, the `teamsLabel_one/_other` unit under the header count and the `teamCount_one/_other` per-country label, plus the empty state (`empty` with `emptyHint`/`emptyHintSearch`) | Participants screen |
...
| `signature` | Signature-capture screen text, incl. the `discardConfirm` dialog shown when switching seats with an unconfirmed drawing (single-brace `{name}`/`{total}` placeholders, filled with `.replace`) | Signature screen |
| `lottery` | Photo-lottery screen: take/add/upload-photo actions, delete confirmation | Photo lottery screen |
| `lotteryOptions` | Options-lottery editor & puller: add/edit lottery form, option list fields, `rePullConfirm`/`clearResultsConfirm`/`deleteConfirm` dialogs, and an `errors` object keyed by validation error code (`no-options`, `invalid-pulls-per-table`, `missing-title`, `invalid-weight`, `invalid-max-per-table`, `insufficient-capacity`) interpolated with `{{title}}`/`{{pullsPerTable}}`/`{{capacity}}` | Options-lottery editor/puller screens |
